...
void register_aircraft(db_aircraft_t *db_aircraft, aircraft_t *aircraft,
  ...
  // Insere no inicio ou no fim conforme o modo
...
// Insere uma rota na lista encadeada
...
aircraft_t *new_aircraft(db_aircraft_t *db_aircraft) {
  ...
  // Define capacidade de passageiros apenas se for do tipo Passageiros
...
// Cria uma nova estrutura de rota solicitando dados ao usuario
routes_t *new_route(db_routes_t *db_routes, db_aircraft_t *db_aircraft) {
  ...
  // Loop para garantir que uma aeronave valida seja selecionada
  ...
  // Valida passageiros de acordo com a capacidade da aeronave
  validate_passengers(
...
void update_aircraft_status(db_aircraft_t *db_aircraft) {
  ...
  // Busca a aeronave (podemos usar a busca linear simples ou a recursiva se
  // incluirmos search.h) Como nao incluimos search.h aqui ainda, vamos fazer
  // linear simples
